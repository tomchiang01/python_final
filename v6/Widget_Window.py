...
class Widget_Window:
    ...
    def remove(self):
        if self.ready:
            ...
            temp_hash = imagehash.phash(Image.open(path+"\\"+self.filename))
            self.image_bad_hash.append(temp_hash)
            os.remove(path+"\\"+self.filename)
            del self.picdict[self.picid]
            self.last()
        ...
        if len(self.picdict) != 0:
            ...
            if self.img_refresh_time != 0:
                try:
                    ...
                    pass
                self.alarm = threading.Timer(self.img_refresh_time * 60, self.refresh_img, args=(1,))
                self.alarm.start()
    ...
    def refresh_picdict(self):
        ...
        try:
            ...
            pass
        self.downloader = search.Search(self.key_word, limit=self.temp_size,  output_dir="temp", black_list=self.image_bad_hash, verbose=True)
        print("refresh picdict.")
        if self.picdict_refresh_time != 0:
            try:
                self.alarm1.cancel()
            except:
                pass
            self.alarm1 = threading.Timer(self.picdict_refresh_time * 60, self.refresh_picdict)
            self.alarm1.start()
        
        
    def set_key_word(self):
        print(self.image_bad_hash)
        path=os.path.abspath(".\\temp")
